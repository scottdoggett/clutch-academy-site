...
**TikTok Pixel (ad measurement, parallel to Meta Pixel)**
- Pixel ID `D87LSEBC77UENKCNER8G` integrated via `src/lib/tiktokPixel.js`, mirroring `metaPixel.js` end-to-end (lazy load, consent gating, prerender guard, idempotent loader, identical helper surface).
- `events.js` is **lazy-loaded only after consent**. Zero requests to `analytics.tiktok.com` before the user clicks Accept.
- Returning visitors with stored consent get the Pixel loaded from `main.jsx` on boot alongside the Meta Pixel; first-time visitors load both from `ConsentBanner.jsx` on accept.
- Events fired from the site:
  - `page` — once after consent (TikTok's equivalent of `PageView`, called inside `loadPixel()`).
  - `ClickButton` with `{ source }` — fired alongside Meta's `Lead` from every Book Now CTA (`hero`, `nav`, `packages_single`, `packages_3pack`, `about`, `reverse`) in `useCalendly.js`. TikTok's `ClickButton` is the standard, conversion-optimizable event for primary CTA intent — it's the closest semantic match to Meta's `Lead`.
  - `Contact` with `{ method }` — fired alongside Meta's `Contact` from phone / email / Instagram / Facebook taps in the Reverse section. TikTok has a standard `Contact` event so the semantics line up exactly.
- **Booking completions (`SubmitForm` / `CompleteRegistration`) are NOT fired from this site.** Same reasoning as Meta — Sam's Calendly-side integration handles booking completions. If Sam wires up Calendly→TikTok the same way as Calendly→Meta, the site shouldn't double-count.
- Consent banner copy updated to disclose both Meta Pixel and TikTok Pixel.
- `public/privacy.html` updated in three places (advertising measurement bullet, no-sale paragraph, third-parties list) with a link to TikTok's privacy policy.
- Call-site changes use explicit aliased imports (`trackMetaLead` / `trackTiktokLead`, `trackMetaContact` / `trackTiktokContact`) rather than a barrel — keeps it visually obvious which pixels are firing from each handler.
- Prerender verified clean: `grep -c -i "tiktok\|analytics.tiktok\|ttq" dist/index.html` returns `0`.
...
#### Blocked on Sam (TikTok Pixel / ad launch)

Surfaced 2026-05-26 when Sam sent TikTok pixel `D87LSEBC77UENKCNER8G` for integration. Browser-side integration is shipped; the items below mirror the Meta open items and need Sam's input or confirmation.

a. **Confirm TikTok Pixel is firing in TikTok Events Manager.** After the next deploy, open TikTok Ads → Events Manager → Test Event for pixel `D87LSEBC77UENKCNER8G`, paste the live URL, click around the site (Book Now CTA + a contact link), and confirm `page`, `ClickButton`, and `Contact` show up within seconds. Consent banner reset (if needed): `localStorage.removeItem('clutch.consent.v1')`.

b. **TikTok Events API (server-side) — does Sam want it?** Equivalent of Meta's CAPI. Browser-only is fine to launch ads, but server-side ingest survives ad-blockers and iOS tracking restrictions and TikTok's optimization improves with the dual signal. Same effort estimate as CAPI (~half a day, either a small serverless endpoint or a connector service like Stape).

c. **Confirm whether Sam's Calendly is wired to TikTok.** Calendly has a native TikTok Pixel integration. If it's on, it should fire `SubmitForm` (or `CompleteRegistration`) for completed bookings to the same pixel `D87LSEBC77UENKCNER8G`. If it's off, completion tracking is missing — but turning it on is a Calendly-dashboard task, not a code task. The site code does NOT fire booking-completion events (same reasoning as Meta — would double-count).

d. **Does Sam want a `ViewContent` event on the Packages section for TikTok?** Same trade-off as the Meta `ViewContent` open item — useful for "viewed pricing but didn't book" retargeting, cheap to add once. If we add it for Meta, add it for TikTok at the same time and fire both from the same IntersectionObserver.

e. **Does Sam want a custom audience for TikTok retargeting?** Once Pixel data starts flowing, Sam can build Custom Audiences in TikTok Ads Manager (site visitors, CTA-clickers who didn't book, etc.) the same way as Meta. Ads Manager task, not a code task — but flag during the launch conversation so he sets up audiences before turning on spend.
...
src/
  ...
  hooks/
    useCalendly.js               <- Lazy-loads Calendly; fires booking_cta_click (GA4) + Lead (Meta) + ClickButton (TikTok)
  lib/
    metaPixel.js                 <- Loads fbevents.js on consent + trackLead/trackContact helpers
    tiktokPixel.js               <- Loads TikTok events.js on consent + trackLead/trackContact helpers
docs/
  seo.md                         <- This file
...
|---|---|---|---|
| Loaded when? | Library loads on every page; Consent Mode suppresses data | Library not loaded at all until consent | Library not loaded at all until consent |
| Used for | Site analytics | Ad performance + audience building | Ad performance + audience building |
| Custom event | `booking_cta_click` | `Lead`, `Contact` | `ClickButton`, `Contact` |
| ID | `G-5E5GEN5N59` (2 places in `index.html`) | `2845684255788584` (in `src/lib/metaPixel.js`) | `D87LSEBC77UENKCNER8G` (in `src/lib/tiktokPixel.js`) |
| Owns the account | Sam's partner (shared Google account) | Sam (Meta Business) | Sam (TikTok Ads) |
...
Sam also runs TikTok ads (added 2026-05-26). The TikTok Pixel is wired in parallel to the Meta Pixel — same consent gate, same call sites, same prerender guard — so the two ad platforms see the same set of intent signals from the site. The Pixel ID is `D87LSEBC77UENKCNER8G` (hardcoded in `src/lib/tiktokPixel.js` — change there if it ever rotates).

### Architecture

Identical to the Meta Pixel by design:

1. `src/lib/tiktokPixel.js` exports `loadPixel()`, `trackLead(source)`, and `trackContact(method)`.
2. On app boot in `src/main.jsx`, if `localStorage['clutch.consent.v1'] === 'granted'` (returning visitor), `loadPixel()` fires immediately alongside the Meta Pixel's `loadPixel()`.
3. On first-time accept in `src/components/ConsentBanner.jsx`, both pixels' `loadPixel()` run together.
4. `loadPixel()` is idempotent (guards on a module-level `loaded` flag) and short-circuits during the build-time prerender (`window.__PRERENDER__`). It defines the standard TikTok `ttq` queue, injects `https://analytics.tiktok.com/i18n/pixel/events.js`, then calls `ttq.load(PIXEL_ID)` + `ttq.page()`.
5. The track helpers (`trackLead`, `trackContact`) guard with `typeof window.ttq?.track !== 'function'` so they no-op silently before consent.

The prerender output `dist/index.html` contains **zero** TikTok Pixel artifacts — verified with `grep -c -i "tiktok\|analytics.tiktok\|ttq" dist/index.html` returning `0`.

### Events fired from this site

| Event | Where it fires | Params |
|---|---|---|
| `page` | Once, after consent — inside `loadPixel()` (TikTok's equivalent of `PageView`) | — |
| `ClickButton` | `useCalendly.js` → `openCalendly(source)` — every Book Now CTA click that opens Calendly | `{ source: "hero" \| "nav" \| "packages_single" \| "packages_3pack" \| "about" \| "reverse" }` |
| `Contact` | `Reverse.jsx` — click on phone / email / Instagram / Facebook contact links | `{ method: "phone" \| "email" \| "instagram" \| "facebook" }` |

### Why `ClickButton` and not `Lead` or `SubmitForm`

TikTok's standard event catalog is: `AddPaymentInfo`, `AddToCart`, `AddToWishlist`, **`ClickButton`**, `CompletePayment`, `CompleteRegistration`, **`Contact`**, `Download`, `InitiateCheckout`, `PlaceAnOrder`, `Search`, `SubmitForm`, `Subscribe`, `ViewContent`. There is no standard `Lead` event the way Meta has one.

The site fires on the CTA click — *before* any form is submitted (Calendly handles the form). The two reasonable choices are `ClickButton` (the literal action) or `SubmitForm` (the conversion intent). `ClickButton` won because:

- It's the literal action; `SubmitForm` would be a lie about what just happened.
- TikTok Ads Manager can optimize a campaign for `ClickButton` directly.
- If/when Sam wants completed-booking tracking he can fire `SubmitForm` from his Calendly-side integration (matching how `Schedule` is wired on the Meta side), keeping the funnel clean: `ClickButton` (site) → `SubmitForm` (Calendly completion).

### Events NOT fired from this site

- **`SubmitForm` / `CompleteRegistration` (completed booking).** Same reason as Meta — Calendly handles booking completions on Sam's side. Adding them here would either double-count (if Calendly→TikTok is wired) or fire from the wrong domain (if Calendly→TikTok is not wired but the user wants completion tracking — that needs to be set up Calendly-side, not site-side).
- **`ViewContent` (saw pricing).** Cheap to add via IntersectionObserver on the Packages section, but not requested. Same trade-off as the Meta side — useful for retargeting "viewed pricing but didn't book."
- **Server-side events via TikTok Events API (the equivalent of Meta's CAPI).** Browser-only setup. TikTok also supports a server-side ingest that survives ad-blockers + iOS tracking restrictions, but it needs a small backend the same way CAPI does.

### How this differs from Meta Pixel

The integrations are intentionally near-identical. The only real differences are vendor-specific:

| | Meta Pixel | TikTok Pixel |
|---|---|---|
| Loaded when? | After consent | After consent |
| Library | `connect.facebook.net/en_US/fbevents.js` | `analytics.tiktok.com/i18n/pixel/events.js` |
| Global queue | `window.fbq` | `window.ttq` |
| Page-view call | `fbq('track', 'PageView')` | `ttq.page()` |
| Lead-equivalent event | `Lead` | `ClickButton` |
| Contact event | `Contact` | `Contact` |
| Pixel ID | `2845684255788584` (in `src/lib/metaPixel.js`) | `D87LSEBC77UENKCNER8G` (in `src/lib/tiktokPixel.js`) |
| Owns the account | Sam (Meta Business) | Sam (TikTok Ads) |

### Adding new event firings

Pattern is identical to Meta. To add a new firing site that should hit both pixels, import both helpers with aliased names and call them together — that's how `useCalendly.js` and `Reverse.jsx` are wired. If you find yourself adding a third pixel, consider collapsing into a `src/lib/pixels.js` barrel that fans out — but for two pixels, explicit aliased imports are the least-magic option and make the call sites self-documenting.
...
| Update the TikTok Pixel ID | `src/lib/tiktokPixel.js` — `PIXEL_ID` constant. Single source of truth. |
| Add a new TikTok Pixel event | Import a helper from `src/lib/tiktokPixel.js` in the calling component. Match TikTok's standard event names (`ClickButton`, `Contact`, `SubmitForm`, etc.) for Ads Manager optimizability. |
...
# Prerender output is TikTok-Pixel-free (same gate as Meta)
curl -s https://clutchacademy.ca/ | grep -c -i -E "ttq|analytics\.tiktok|D87LSEBC77UENKCNER8G"  # expect 0
...
- **TikTok Events Manager → Test Event** (for pixel `D87LSEBC77UENKCNER8G`) — same idea as Meta. Paste the live URL, accept consent, click around. `page` fires immediately on load; `ClickButton` fires from every Book Now CTA; `Contact` fires from the Reverse contact links.
- **TikTok Pixel Helper** (Chrome extension by TikTok) — equivalent to Meta Pixel Helper. Confirms the pixel is firing without opening Events Manager.
...
- **Don't add a `Schedule` / `CompleteRegistration` event for completed Calendly bookings.** Sam wired up Calendly's native Meta integration on his end, so the booking-completion conversion is already firing from Calendly's side. Adding one from the site would double-count in Ads Manager and mis-train Meta's ad-optimization algorithm. If Sam ever turns off the Calendly-side integration, *then* add a `postMessage` listener for Calendly's `calendly.event_scheduled` message inside `useCalendly.js`. Same logic applies to TikTok's `SubmitForm` / `CompleteRegistration` — if Sam wires Calendly→TikTok, do not fire a duplicate from the site.
- **Don't load `fbevents.js` or TikTok's `events.js` unconditionally from `index.html`.** Both pixels are intentionally not loaded before consent — moving either to `<head>` breaks the consent gate and creates PIPEDA / GDPR exposure. The lazy-load pattern in `src/lib/metaPixel.js` / `src/lib/tiktokPixel.js` is the supported pattern.
- **Don't change either Pixel ID without telling Sam.** Meta ID lives in `src/lib/metaPixel.js`; TikTok ID lives in `src/lib/tiktokPixel.js`. If either is wrong, Sam's ad data flows to a dataset he can't see — symptoms are "no events showing up in Events Manager" despite `fbq` / `ttq` running fine in the browser console. Mismatched Pixel ID is the #1 cause of "the Pixel doesn't work."
- **Keep Meta and TikTok event payloads in sync.** Both pixels fire from the same call sites (`useCalendly.js` and `Reverse.jsx`) and currently receive identical `{ source }` / `{ method }` params. If you add a new CTA, add it to both call sites — silently dropping one platform's call produces an attribution gap that's hard to spot in dashboards.
