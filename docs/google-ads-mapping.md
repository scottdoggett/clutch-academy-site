...
`lessons_overview_close`, so Ads pointed at that URL attribute to the hub rather
than leaking to whichever package page the visitor clicked into — which was the
case between the intro hero's removal and August 18, 2026. The chooser below it
routes rather than books: its rows link to package pages, and a visitor who books
from one attributes to that page's tag. The `lessons_overview_pick_*` tags that
briefly labelled per-row Book buttons are retired along with the buttons, as is
the older `lessons_overview`, which labelled the removed hero's button.
